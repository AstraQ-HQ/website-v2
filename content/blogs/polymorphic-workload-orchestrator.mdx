...
In <EmbedResource resourceType="blog" slug="secure-microvm-runtime" placeholder="Part 2" />, we will implement the `FirecrackerRuntime` struct. We'll show how to use the exact same interface to spin up micro-VMs that offer hardware-level isolation for hostile workloads, without changing a single line of our API code.
