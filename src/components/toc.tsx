  ...
  return (
    <div className="hidden md:block sticky top-24 h-fit">
      <div className="rounded-lg p-4">
        <div
          ref={containerRef}
          className="relative overflow-y-auto max-h-[calc(100vh-16rem)]"
        >
          <p className="text-xl font-mono sticky top-0 z-10 bg-card">
            On this page
          </p>
          {isOverflowing && (
            <div className="sticky top-4 left-0 right-0 h-12 bg-linear-to-b from-card to-transparent pointer-events-none" />
          )}
          <ul className="space-y-1">
            {headings.map((heading) => (
          ...
          {isOverflowing && (
            <div className="sticky -bottom-4 left-0 right-0 rounded-b-lg h-12 bg-linear-to-t from-card to-transparent pointer-events-none" />
          )}
        </div>
      </div>
    </div>
